...
LANGUAGES.forEach((lang) => {
  test(`a new event's basic info (page 1/3) can be added using language ${lang}`, async ({
    ...
    await p1.fillRichEditorTextBox(p1.descriptionTextBox, 'Pitkä kuvaus');
    ...
    await p1.clickListBoxOption(p1.eventKeywordsListBox, p1.t('music'));
    ...
    const createEventRequestPromise = p1.createEventRequestPromise();
    ...
    const actualVariables = createEventRequest.postDataJSON().variables;
    ...
    expect(actualVariables).toEqual(expect.objectContaining(expectedVariables));

    const startTime = actualVariables?.event?.startTime;
    const contactPersonId = actualVariables?.event?.pEvent?.contactPersonId;
    const organisationId = actualVariables?.event?.organisationId;
