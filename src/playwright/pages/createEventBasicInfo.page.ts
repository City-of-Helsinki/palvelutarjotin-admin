...
export class CreateEventBasicInfoPage extends AuthenticatedPage {
  ...
  /**
   * Wait for the request that is sent when the CreateEvent mutation is called
   * in order to be able to verify its variables.
   */
  createEventRequestPromise() {
    return this.page.waitForRequest(
      (request: Request) =>
        request.url().includes('/graphql') &&
        request.method() === 'POST' &&
        request.postDataJSON()?.operationName === 'CreateEvent'
    );
  }

  // protected isRichEditorTextBox(textBoxName: TextBoxName) {
  //   return textBoxName == 'description';
  // }
}
